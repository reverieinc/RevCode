...
def normalizeAllLang_Revcode(string_1):
    ...
    while i <strlen:
        ch = ""
        if (i<strlen-1) and (string_1[i] == 'W' or string_1[i] == 'H' or string_1[i] == 'F' or string_1[i] == 'Y'):
            ch += string_1[i]
            ch += string_1[i + 1]
            i = i + 2
        else:
        ...
        if ch == "M":
            ...
            output += "i"
            if (i + 1 < strlen -1) and string_1[i] == "y" and string_1[i + 1] == "O":
                output += "o"
                i = i + 2
...
def revcode_normalizer(rev_string, level) :

    rev_string_list = rev_string.split()
    normalized_rev_string_list = []
    for rev_string in rev_string_list :
        normalized_rev_string_list.append(normalize_revcode(rev_string, level))


    normalized_rev_string = " ".join(normalized_rev_string_list)
    return normalized_rev_string


def normalize_revcode(rev_string, normalization_level):
    rev_string = rev_string.strip()
    rev_string = rev_string.replace(chr(0x200c), "")

    if normalization_level == 0:
        return rev_string

    elif normalization_level == 1:
        rev_string = normalizeAllLang_Revcode(rev_string)
        return rev_string

    elif normalization_level == 2:
        rev_string = normalizeAllLang_Revcode(rev_string)
        for ch in ["a", "e", "i", "o", "u", "A", "E", "I", "O", "U"]:
            rev_string = rev_string.replace(ch, "")
        return rev_string
...
def main():
    ...
    file = codecs.open("/Users/ReveriePC/work-reverie/git/kkch/kkch/data/hindi.txt/food_names", "r", "utf-8")
    ...
    for line in file:
        ...
        for i in range(len(line_t)):
            # particular language index
            if i != 2:
                line_out.append(line_t[i])
            else:
                normalised_word = revcode_normalizer(line_t[i])
                line_out.append(normalised_word)
        line =  "\t".join(line_out)
        print (line,)
...
if __name__ == '__main__':
    print(revcode_normalizer("vAI-PAi", 1))
